...
async def account_login(bot: Client, m: Message):
    ...
    rwa_url = "https://api.penpencil.co/v1/oauth/token"  
    input1: Message = await bot.listen(editable.chat.id)
    raw_text = input1.text
    
    headers = {
            'Host': 'api.penpencil.co',
            'authorization': 'Bearer 80b7c9e5f67f88ae703c4fcc5edfec4785ff9fec550aceca8dc7a920274fa27a',
  ...
  "grant_type": "password"}
    ...
    response = s.get('https://api.penpencil.co/v3/batches/my-batches', params=params, headers=headers).json()["data"]
    cool = ""
    mm = "KhanSir"
    ...
    response2 = s.get(f'https://api.penpencil.co/v3/batches/{raw_text3}/details', headers=headers).json()["data"]
    response3 = s.get(f'https://api.penpencil.co/v3/batches/{raw_text3}/details', headers=headers).json()["data"]["subjects"]
    ...
    response02 = s.get(f'https://api.penpencil.co/v2/batches/{raw_text3}/subject/{raw_text4}/topics?page=1', headers=headers).json()["data"]
    ...
    for y in range(0,len(xv)):
      t =xv[y].strip()
      html3 = s.get("https://api.penpencil.co/v2/batches/"+raw_text3+"/subject/"+raw_text4+"/contents?page=1&tag="+t+"&contentType=videos",headers=headers).content
      ff = json.loads(html3)
      tpage = (ff["paginate"])["totalCount"]//ff["paginate"]["limit"]+2
      print("Total page:",tpage)
      for i in range(1,tpage)[::-1]:
        html4 = s.get("https://api.penpencil.co/v2/batches/"+raw_text3+"/subject/"+raw_text4+"/contents?page="+str(i)+"&tag="+t+"&contentType=videos",headers=headers).json()["data"]
        html4.reverse()
        #break
        for dat in html4:
          try:
